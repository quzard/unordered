...
测试固定插入 10 万个元素，通过 `reserve` 预设不同的桶数量来控制**目标负载因子**，然后测试查找 10 万次的时间。
...
|                           |     0.3 |   0.288 |            85 |
|                           |     0.5 |   0.480 |            85 |
|                           |     0.7 |   0.480 |            85 | *注意：实际 LF 未达到 0.7*
|                           |     0.8 |   0.480 |            85 | *注意：实际 LF 未达到 0.8*
|                           |     0.9 |   0.480 |            86 | *注意：实际 LF 未达到 0.9*
|                           |    0.95 |   0.480 |            85 | *注意：实际 LF 未达到 0.95*
...
1.  **`boost::unordered_flat_map` 的最佳区间**: 在 0.5 到 0.8 的负载因子区间内，`flat_map` 的查找性能达到最佳且非常稳定（约 58ms）。即使在更高的负载因子（0.9-0.95）下，性能下降也非常有限。这体现了其高效的探测机制和对较高负载的良好容忍度。
2.  **`std::unordered_map` 的表现**: 在低负载因子（0.1）下性能较差，随着负载因子增加，性能逐渐提升，在 0.7 左右达到最佳（约 93ms）。之后性能略有下降，但总体对高负载因子不如 `flat_map` 敏感。
3.  **`boost::unordered_map` 的特殊行为**: 测试数据显示，当目标负载因子超过 0.5 时，`boost::unordered_map` 的实际负载因子始终维持在 0.48 左右。这强烈暗示其内部的 `reserve` 或插入逻辑包含了一种**自动的 rehash 策略**，不允许负载因子超过某个内部阈值（可能是 0.5 或略高），从而保证了其在不同目标负载因子设定下性能的高度稳定（约 85ms）。这是一种有趣的优化，牺牲了空间换取了更可预测的性能。
4.  **低负载因子的代价**: 所有类型的 map 在极低负载因子（0.1）下性能都相对较差，因为大量的空间被浪费，导致需要遍历的桶/槽位更多，缓存效率也可能下降。

### 5.3 多线程性能对比

测试包含 100 万元素，使用 8 个线程执行 1000 万次操作。非并发 map 使用 `std::shared_mutex` 加锁。

**测试结果 (`multi_thread_results.txt`)**: 

| Map 类型                       | 单线程插入 (ms) | 单线程查找 (ms) | 多线程插入 (ms) | 多线程查找 (ms) | 多线程混合,70% 查找, 20% 插入, 10% 删除 (ms) |
| :----------------------------- | --------------: | --------------: | --------------: | --------------: | --------------: |
| `std::unordered_map` (locked)    |             386 |            1887 |            2204 |            4197 |            3651 |
| `boost::unordered_map` (locked)  |             398 |            1834 |            2152 |            3741 |            3502 |
| `boost::unordered_flat_map` (locked)|             270 |            1141 |            2390 |            4429 |            4163 |
| `boost::concurrent_flat_map`   |             277 |            1134 |             777 |             169 |             273 |

**数据分析**: 

1.  **`boost::concurrent_flat_map` 的压倒性优势**: 这是最显著的发现。
    *   多线程插入：比加锁的 `flat_map` 快约 **67.5%** (`(2390-777)/2390`)，比加锁的 `std/boost` 快约 **64-65%**。
    *   多线程查找：比加锁的 `flat_map` 快约 **96%** (`(4429-169)/4429`)！比加锁的 `std/boost` 快约 **95-96%**！速度提升超过 **20 倍**！
    *   多线程混合操作：比加锁的 `flat_map` 快约 **93%** (`(4163-273)/4163`)！比加锁的 `std/boost` 快约 **92%**！速度提升超过 **12 倍**！
    *   这些数据有力地证明了 `concurrent_flat_map` 通过**分段锁**和**无锁优化**（如乐观插入）有效降低了线程间的争用，实现了极高的并发吞吐量。其 `visit` API 也保证了操作的原子性和安全性。
2.  **锁竞争成为瓶颈**: 对于加了外部锁的 `std::unordered_map`, `boost::unordered_map` 和 `boost::unordered_flat_map`，它们的多线程性能相比单线程反而显著下降（时间增加数倍）。这清晰地表明，在高并发下，`std::shared_mutex` 成为了严重的瓶颈，线程大部分时间可能都在等待锁，而不是执行有效操作。
3.  **加锁 `flat_map` 的反常表现**: 有趣的是，虽然 `flat_map` 在单线程下最快，但在加了全局读写锁的多线程环境下，其性能（尤其查找和混合操作）甚至略差于加锁的 `std::unordered_map` 和 `boost::unordered_map`。一个可能的解释是，`flat_map` 极度依赖缓存局部性，而全局锁可能导致更频繁的缓存一致性协议流量（Cache Coherence Traffic）和缓存行失效，尤其是在不同 CPU 核心上的线程争抢同一个锁时，这种负面影响可能抵消了其单线程的优势。相比之下，基于节点分离链表的 map 可能对这种缓存颠簸不那么敏感。
4.  **单线程开销**: `concurrent_flat_map` 的单线程性能与 `unordered_flat_map` 非常接近，表明其内部并发控制机制在无竞争情况下的开销非常小。
...
Boost.Unordered 库的不同实现确实针对不同场景进行了深度优化：

*   `boost::unordered_map` 是对 `std` 的温和改进，单线程性能更好。
*   `boost::unordered_flat_map` 通过开放寻址和 SIMD 优化，在**单线程**场景下提供了卓越的性能。
*   `boost::concurrent_flat_map` 通过精巧的**分段锁**和 **`visit` API** 设计，在**高并发**场景下展现了压倒性的性能优势，解决了传统锁机制的瓶颈。

基准测试结果清晰地表明，根据应用场景（单线程 vs. 高并发）选择合适的容器至关重要，其性能差异可以是数量级的。在高并发服务或计算中，盲目使用标准库容器加外部锁是不可取的，应优先考虑使用 `boost::concurrent_flat_map` 或类似的专为并发设计的容器。
